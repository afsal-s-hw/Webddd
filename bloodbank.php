...
// Fetch blood banks
$banks = [];
$sql = "SELECT * FROM bloodbanks ORDER BY name ASC";
...
<!-- Hero Section -->
<section class="hero">
    <div class="badge">❤ Blood Banks</div>
    <h1>Find a Blood Bank <span>Near You</span></h1>
    <p>Connect with verified blood banks in your city and get blood or plasma when needed.</p>
</section>

<!-- Blood Banks Grid -->
<section class="section">
<div class="section-header"><h2>Registered Blood Banks</h2></div>
<div class="donor-grid">
<?php if(count($banks) > 0): ?>
    <?php foreach($banks as $b): ?>
        <div class="card">
            <div class="profile-img">
                <?php if(!empty($b['photo']) && file_exists("uploads/".$b['photo'])): ?>
                    <img src="uploads/<?= $b['photo'] ?>" alt="<?= $b['name'] ?>">
                <?php else: ?>
                    <?= strtoupper(substr($b['name'],0,1)) ?>
                <?php endif; ?>
            </div>
            <div class="name"><?= $b['name'] ?></div>
            <div class="location">📍 <?= $b['city'] ?></div>
            <div class="contact"><?= $b['phone'] ?? 'N/A' ?></div>
        </div>
    <?php endforeach; ?>
<?php else: ?>
    <p style="grid-column:1/-1;text-align:center;">No blood banks registered.</p>
<?php endif; ?>
</div>
</section>
